...
const examples: { [key: string]: { name: string, image: string } } = {
...
function createButtons() {
    ...
    const allKeys = Object.keys(examples);
    allKeys.forEach(key => {
...
function displayExample(example: { name: string, image: string }) {
    const exampleDiv = document.getElementById("example")!;
    exampleDiv.innerHTML = `<h2>${example.name}</h2><div class="placeholder">Loading...</div>`;
    
    // Lazy load the image
    const img = new Image();
    img.src = example.image;
    img.onload = () => {
        exampleDiv.innerHTML = `<h2>${example.name}</h2><img src="${example.image}" alt="${example.name}" />`;
    };
    img.onerror = () => {
        exampleDiv.innerHTML = `<h2>${example.name}</h2><p>Image failed to load</p>`;
    };
}

// Change button color on click
function changeButtonColor(button: HTMLButtonElement) {
    button.style.backgroundColor = "#ff6347"; // Change to a different color
}

// Play fireworks effect on button click
function playFireworks(button: HTMLButtonElement) {
    const firework = document.createElement("div");
    firework.classList.add("firework");
    document.body.appendChild(firework);

    const buttonRect = button.getBoundingClientRect();
    firework.style.left = `${buttonRect.left + buttonRect.width / 2 - 10}px`;
    firework.style.top = `${buttonRect.top + buttonRect.height / 2 - 10}px`;
